...
def read_keypoints(keypoint_fn, use_hands=True, use_face=True, use_face_contour=True):
    ...
    for idx, person_data in enumerate(data["people"]):
        ...
        if use_hands:
            ...
            right_hand_keyp = np.array(person_data["hand_right_keypoints_2d"], dtype=np.float32).reshape([-1, 3])

            body_keypoints = np.concatenate([body_keypoints, left_hand_keyp, right_hand_keyp], axis=0)
        if use_face:
            # TODO: Make parameters, 17 is the offset for the eye brows,
            # etc. 51 is the total number of FLAME compatible landmarks
            face_keypoints = np.array(person_data["face_keypoints_2d"], dtype=np.float32).reshape([-1, 3])[
                17 : 17 + 51, :
            ]

            contour_keyps = np.array([], dtype=body_keypoints.dtype).reshape(0, 3)
            if use_face_contour:
                contour_keyps = np.array(person_data["face_keypoints_2d"], dtype=np.float32).reshape([-1, 3])[:17, :]
...
class OpenPose(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.img_paths = [
            osp.join(self.img_folder, img_fn)
    ...
    def get_joint_weights(self):
        ...
        optim_weights = np.ones(
            self.num_joints + 2 * self.use_hands + self.use_face * 51 + 17 * self.use_face_contour, dtype=np.float32
        )
    ...
    def read_item(self, img_path):
        ...
        # return output_dict
        keypoint_fn = osp.join(self.keyp_folder, f"{img_fn}.json")
        ...
        output_dict = {"fn": img_fn, "img_path": img_path, "img": img, "keypoints": keypoints}
