...
def fit_single_frame(
    ...
    **kwargs,
    ...
    with fitting.FittingMonitor(batch_size=batch_size, visualize=visualize, **kwargs) as monitor:
        ...
        camera_loss.reset_loss_weights({"data_weight": data_weight}, -1)
        ...
        for or_idx, orient in enumerate(tqdm(orientations, desc="Orientation")):
            ...
            for opt_idx, curr_weights in enumerate(tqdm(opt_weights, desc="Stage")):
                ...
                loss.reset_loss_weights(curr_weights, opt_idx)
                ...
                if save_meshes:
                    ...
                    result_opt_fn = result_fn.replace(".pkl", f"_opt_{opt_idx}.pkl")
                    with open(result_opt_fn, "wb") as result_file:
                        pickle.dump(results, result_file, protocol=2)
                    print(f"Saved results to {result_opt_fn}")
                    ...
                    # print(body_model.output)
                    out_mesh.apply_transform(rot)
                    mesh_opt_fn = mesh_fn.replace(".obj", f"_opt_{opt_idx}.obj")
                    out_mesh.export(mesh_opt_fn)
                    print(f"Saved mesh to {mesh_opt_fn}")
