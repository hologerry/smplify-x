...
def main(**args):
    ...
    for vid_dict in tqdm(cur_split_dicts, total=len(cur_split_dicts), leave=False, desc="Processing videos"):
        ...
        frames_folder = os.path.join(data_folder, "frames", video_name)
        keypoints_folder = os.path.join(data_folder, "keypoints", video_name)
        dataset_obj = create_dataset(img_folder=frames_folder, keyp_folder=keypoints_folder, **args)
        ...
        for idx, data in enumerate(dataset_obj):
            ...
            (cur_body_pose, cur_camera_transl, cur_camera_orient, cur_betas_fix, nan_flag,) = fit_single_frame(
                img,
                idx,
                keypoints,
